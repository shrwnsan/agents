...
# ADE-48: file command is optional — unknown types get application/octet-stream
...
# ADE-49: Warn if credentials file is world-readable
...
# ADE-45: Dangerous extensions — always blocked
...
# ADE-46: Suspicious extensions — warn unless --allow-suspicious
...
guess_content_type() {
  ...
  case "${f##*.}" in
    ...
    bmp)      echo "image/bmp" ;;
    *)
      # ADE-48: No file command dependency — unknown types get generic MIME
      echo "application/octet-stream" ;;
  esac
}

# ADE-47: Pre-upload secret scanning — check file content for leaked credentials
scan_for_secrets() {
  local f="$1"
  local rel="$2"
  local issues=0
...
if [[ -f "$TARGET" ]]; then
  ...
  while IFS= read -r -d '' f; do
    ...
    # ADE-45: Block dangerous extensions
    if is_dangerous_extension "$ext" "$bn"; then
      BLOCKED_FILES+=("$rel")
    ...
    # ADE-46: Warn on suspicious extensions
    if is_suspicious_extension "$ext" "$bn"; then
      if [[ "$ALLOW_SUSPICIOUS" -ne 1 ]]; then
        BLOCKED_FILES+=("$rel")
    ...
    # ADE-48: Warn on truly unknown extensions
    if ! is_known_extension "$ext" && ! is_suspicious_extension "$ext" "$bn"; then
      if [[ "$ALLOW_UNKNOWN" -ne 1 ]]; then
        BLOCKED_FILES+=("$rel")
    ...
    # ADE-47: Pre-upload secret scanning
